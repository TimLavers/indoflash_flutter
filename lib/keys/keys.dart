import 'package:flutter/widgets.dart';

const homeScreen = Key('_home_screen_');
const wordListSelectorLauncher = Key('_word_list_selector_launcher_');
const chapterSelectorKey = Key('_chapter_selector_');
const wordKey = Key('__word__');
const definitionKey = Key('__definition__');
const navigateListButtonKey = Key('__navigate_list_button__');
const shuffleToggleButtonKey = Key('__shuffle_toggle_button__');
